...
def align_folder(folder_path, folder_save_path, landmark_path):
    ...
    for img in preds.keys():
        ...
        input_image = io.imread(img)
        output_preds = fa.get_landmarks(input_image)

        plt.imshow(input_image)
        plt.scatter(output_preds[0][:, 0], output_preds[0][:, 1], c='white', marker='.')
        os.makedirs(landmark_path, exist_ok=True)
        plt.savefig(landmark_path + '/{}.jpg'.format(file_number))  # Save the image with landmarks
        plt.clf() 

        pred_points = np.array(preds[img])
        if pred_points is None or len(pred_points.shape) != 3:
            print('preprocessing failed_1')
            return False
        else:
...
def main():
    ...
    if os.path.isdir(args.folder_path):
        ...
        align_folder(args.folder_path, save_img_path, os.path.join(home_path, args.folder_path.split('/')[-1] + '_landmark'))
